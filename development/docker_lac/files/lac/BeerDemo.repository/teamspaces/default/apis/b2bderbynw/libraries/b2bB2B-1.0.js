...
B2B.transformToWebHook = function transformToWebHook(aLogicContext, aResourceName, aTargetUrl) {
    ...
    var resourceGetResponse = SysUtility.getResource(aResourceName, options);
    ...
    if (resourceURL === null || resourceURL === "") {
        ...
        var postResponse = SysUtility.restPost(resourceURL, null, ConstantsLib.supplierAuth, resourceGetResponse);
...
// typical technique to outboard constants from rules (see Order.discountedAmount)
// table-based parameters can be altered using Data Explorer
// though, give consideration to caching

B2B.discountRuleParms = null;
B2B.getDiscountRuleParms = function getDiscountRuleParms() {
    var useCache = true;
    if (useCache === false || B2B.discountRuleParms === null) {
        B2B.discountRuleParms = SysUtility.getResource("ruleparameters", {sysfilter: "equal(ident:" + 1 + ")"} );
        out = java.lang.System.out;
        out.println("B2B.discountRuleParms: " + JSON.stringify(B2B.discountRuleParms) );
    }
    return B2B.discountRuleParms[0];
};
...
var ConstantsLib = {    // preferably, load values from property files, as in Conference - Management
    supplierAuth: { headers: { Authorization: "CALiveAPICreator supplier:1" }},  // see Auth Tokens screen
    b2bAuth: { headers: { Authorization: "CALiveAPICreator Bzn8jVyfOTiIpW6UQCgy:1" }}
};
