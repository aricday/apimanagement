...
if (webHooks !== null) {
    ...
    for each (var url in webHooks.values()) {
        ...
        var resourceGetResponse = SysUtility.getResource("SupplierAlert", options);

        // system console output
        logicContext.logDebug("Post SupplierAlert posting getResponse with URL: " + resourceURL +
            "\npayload: " +JSON.stringify(resourceGetResponse));
        if (resourceGetResponse[0].AmountTotal === null)
            throw new Error("System Error - AmountTotal has unexpected null value");
