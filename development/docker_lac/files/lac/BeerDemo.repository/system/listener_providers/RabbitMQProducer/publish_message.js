...
/* Form URL */
var rabbitMQCamelURI = providerUtil.formCamelURI(context, "rabbitmq",
    connectionParameters.hostname +
    ":" + connectionParameters.portNumber +
    "/" + connectionParameters.exchangeName
    );
...
try {
    if (typeof headerMap != 'undefined' && !headerMap.isEmpty()) {
        producer.sendBodyAndHeaders(rabbitMQCamelURI,message,headerMap);
    }
    else {
        producer.sendBody(rabbitMQCamelURI,message);
    }
}
catch(e) {
